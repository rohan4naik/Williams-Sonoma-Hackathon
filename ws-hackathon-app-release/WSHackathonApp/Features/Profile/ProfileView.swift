...
struct ProfileContentView: View {
    ...
    private var email: String {
        mockUserManager.currentUser.email
    }
    
    private var initials: String {
        mockUserManager.currentUser.avatarInitials
    }
    ...
    var body: some View {
        List {
            ...
            Section(header: Text("Saved Addresses")) {
                if let selectedAddress = mockUserManager.dataForCurrentUser().savedAddresses.first(where: { $0.isSelected }) {
            ...
            Section(header: Text("My Orders")) {
                ...
                NavigationLink(destination: ProfilePastOrdersView().environmentObject(mockUserManager)) {
                    HStack {
                        ...
                        Text("Past Orders")
                        Spacer()
                        let ordersCount = mockUserManager.dataForCurrentUser().pastOrders.count
            ...
            // SWITCH ACCOUNT SECTION (TEMPORARY DEV SWITCHER)
            Section {
                HStack {
                    Label("Developer Switcher", systemImage: "person.badge.key")
                        .foregroundColor(.secondary)
                        .font(.footnote)
                    ...
                    Menu {
                        ForEach(mockUserManager.allUsers) { user in
                            Button(action: {
                                withAnimation {
                                    mockUserManager.performSwitch(
                                        to: user,
                                        cartRepo: cartRepo,
                                ...
                                HStack {
                                    Text(user.name)
                                    if mockUserManager.currentUser.id == user.id {
                ...
                Text("This account switcher is for demonstration purposes only.")
                    .font(.caption2)
                    .foregroundColor(.secondary)
            }
        }
...
struct ProfileEditView: View {
    ...
    var body: some View {
            ...
            ToolbarItem(placement: .navigationBarTrailing) {
                Button("Save") {
                    mockUserManager.currentUser = MockUser(
                        id: mockUserManager.currentUser.id,
                        name: name,
                        email: email,
                        avatarInitials: {
                            let first = name.split(separator: " ").first.map(String.init) ?? "D"
                            let last = name.split(separator: " ").last.map(String.init) ?? "U"
                            return String(first.prefix(1) + last.prefix(1)).uppercased()
                        }()
                    )
                    mockUserManager.userDataStore[mockUserManager.currentUser.id]?.phoneNumber = phone
        ...
        .onAppear {
            ...
            email = mockUserManager.currentUser.email
            phone = mockUserManager.dataForCurrentUser().phoneNumber
        }
    }
}
...
struct ProfilePastOrdersView: View {
    ...
    private var simulatedOrders: [SimulatedOrder] {
        mockUserManager.dataForCurrentUser().pastOrders
    }
